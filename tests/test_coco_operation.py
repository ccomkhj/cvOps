...
def test_merge_command(coco_data_detections, coco_data_segmentations, tmp_path):
    ...
    img_dir1, ann_file1, img_dir2, ann_file2 = Path(img_dir1), Path(ann_file1), Path(img_dir2), Path(ann_file2)
    ...
    total_img_count_dir1 = sum(1 for _ in (merged_img_dir / "part1").iterdir() if _.is_file())
    total_img_count_dir2 = sum(1 for _ in (merged_img_dir / "part1").iterdir() if _.is_file())
    ...
    assert len(merged_data["annotations"]) > 0, "Expected at least one annotation in 'annotations'."
